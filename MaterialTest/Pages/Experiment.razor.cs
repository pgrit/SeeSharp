...
public partial class Experiment : ComponentBase
{
    ...
    void RunExperiment()
    {
        ...
        MonochromeImage[] pdfs = [new(Width, Height), new(Width, Height), new(Width, Height)];
        RgbImage[] values = [new(Width, Height), new(Width, Height), new(Width, Height)];
        select = new float[3];
        totals = new float[3];
        Parallel.For(
            0,
    ...
    void OnFlipRenderClick(FlipViewer.EventArgs args)
    {
        if (args.Control)
        {
            ...
            i =>
            {
                ...
                for (int j = 0; j < Height; ++j)
                {
                    ...
                    for (int k = 0; k < 10; ++k)
                    {
                        ...
                        refimg[i, j] =
                            shader.EvaluateWithCosine(sample.Direction) * 4.0f * float.Pi;
                    }
                }
            }
        );
