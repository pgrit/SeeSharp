...
public static class Scripts
{
    static string ReadResourceText(string filename)
    {
        ...
        var stream =
            assembly.GetManifestResourceStream("SeeSharp.Blazor." + filename)
    ...
    public static readonly string FlipBookScript = $$"""
        <script>
            ...
            {{ReadResourceText("Scripts.flipUtil.js")}}
        </script>
        """;
    ...
    public static readonly string AllScripts =
        FlipBookScript + DownloadScript + WidgetScripts + UpdateImageScript;
    ...
    public static async Task DownloadAsFile(
        IJSRuntime js,
        string filename,
        SeeSharp.Common.HtmlReport report
    ) => await DownloadAsFile(js, filename, report.ToString());

    public static async Task DownloadAsFile(
        this SeeSharp.Common.HtmlReport report,
        IJSRuntime js,
        string filename
    ) => await DownloadAsFile(js, filename, report.ToString());
}
