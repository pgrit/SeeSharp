...
public static class SceneRegistry
{
    ...
    static SceneRegistry()
    {
        ...
        foreach (string path in env.Split(Path.PathSeparator))
        {
            ...
            AddSource(path);
            Logger.Log(
                $"Added scene directory specified in the environment: {path}",
                Verbosity.Debug
            );
        }
    }
    ...
    public static void AddSource(string directoryName)
    {
        lock (directories)
        {
            ...
            if (!directory.Exists)
                Logger.Error(
                    $"Tried to add non-existing directory as a scene source. Check if the path is correct: {directoryName}"
                );
            else
    ...
    public static void AddSourceRelativeToScript(
        string relativePath,
    ...
    public static SceneDirectory Find(string name, string variant = null)
    {
        ...
        {
            Logger.Log(
                $"Scene \"{name}\" not found in any registered repository!",
                Verbosity.Error
            );
            foreach (DirectoryInfo dir in directories)
