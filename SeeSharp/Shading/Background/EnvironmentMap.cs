...
public class EnvironmentMap : Background
{
    ...
    public override RgbColor ComputeTotalPower()
    {
        ...
        for (int row = 0; row < Image.Height; ++row)
        {
            for (int col = 0; col < Image.Width; ++col)
            {
                ...
                totalPower +=
                    Image.GetPixel(col, row) * 4 * MathF.PI / (Image.Width * Image.Height);
            }
        }
        return totalPower;
    }
    ...
    public override float DirectionPdf(Vector3 direction)
    {
        var sphericalDir = WorldToSpherical(direction);
        var pixelCoords = SphericalToPixel(sphericalDir);
        return directionSampler.Pdf(pixelCoords)
            / (MathF.Sin(sphericalDir.Y) * MathF.PI * MathF.PI * 2.0f);
    }
    ...
    public override (Ray, RgbColor, float) SampleRay(Vector2 primaryPos, Vector2 primaryDir)
    {
        ...
        var pos =
            ...
            + SceneRadius
                * (
                    dirSample.Direction // offset outside of the scene
                    + tangent * unitDiscPoint.X // remap unit disc x coordinate
                    + binormal * unitDiscPoint.Y
                ); // remap unit disc y coordinate
    ...
    public static Vector2 WorldToSpherical(Vector3 dir)
    {
    ...
    public static Vector3 SphericalToWorld(Vector2 spherical)
    {
    ...
    public static Vector2 SphericalToPixel(Vector2 sphericalDir) =>
        new Vector2(sphericalDir.X / (2 * MathF.PI), sphericalDir.Y / MathF.PI);

    public static Vector2 PixelToSpherical(Vector2 pixel) =>
