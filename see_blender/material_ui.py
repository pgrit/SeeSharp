...
class SEESHARP_PT_context_material(MaterialButtonsPanel, Panel):
    """
...
class SEESHARP_PT_material(MaterialButtonsPanel, Panel):
    COMPAT_ENGINES = {"SEE_SHARP"}
    bl_label = 'SeeSharp Material'
    ...
    def draw(self, context):
        ...
        mat = context.material.seesharp

        layout = self.layout

        row = layout.row()
        row.label(text="Convert")
        row.operator("seesharp.convert_material")
...
def register():
    ...
    bpy.utils.register_class(SEESHARP_PT_material)

def unregister():
    bpy.utils.unregister_class(SEESHARP_PT_context_material)
    bpy.utils.unregister_class(SEESHARP_PT_material)