...
GROUND_API Ray GenerateCameraRay(int camera, CameraSampleInfo sampleInfo) {
    return InternalRayToApi(globalCameras[camera]->GenerateRay(ApiVectorToInternal(sampleInfo.filmSample),
        ApiVectorToInternal(sampleInfo.lensSample), sampleInfo.time));
}
